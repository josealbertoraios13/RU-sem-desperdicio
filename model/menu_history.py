...
class MenuHistory(Menu):
    def __init__(self, box: window, title: str, width: int, height: int, schedules: list):
    ...
    # Scroll
    def _visible_current_ys(self) -> int:
    ...
    def _adjust_scroll(self) -> None:
        visible = self._visible_current_ys()
    ...
    def _draw_main_container(self) -> None:
        ...
        counter_text = (
            f"Agendamentos para hoje: {today_count}" if today_count > 0 else "Nenhum agendamento para hoje"
        )
        self.box.attron(curses.color_pair(2))
        self.box.addstr(current_y, 5, counter_text[: x - 8])
        self.box.attroff(curses.color_pair(2))
        current_y += 1

        # Linha separadora
        self.box.attron(curses.color_pair(1))
        self.box.addstr(current_y, 2, "─" * (x - 4))
        ...
        if not self._sorted_items:
            ...
            self.box.attron(curses.color_pair(1))
            self.box.addstr(current_y, Utils.get_mid(msg, self.box), msg[: x - 8])
            self.box.attroff(curses.color_pair(1))
            return
        ...
        for list_pos in range(self._scroll_offset, end_idx):
            ...
            if is_sel:
                ...
                self.box.addstr(current_y, 4, label[: x - 6])
